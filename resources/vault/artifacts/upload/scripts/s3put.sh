...
#     s3put.sh <bucket> <key> <path/to/file>
# Role based s3 bucket access
...
debug_log () {
  ...
  -H \"Date: ${dateValue}\"
	https://${bucket}.s3.amazonaws.com/${key}/${filePath} " >> /tmp/s3-bootstrap.log
}
...
curl $opts -H "Host: ${bucket}.s3.amazonaws.com" \
  ...
  -H "x-amz-security-token:${s3Token}" \
  -H "Date: ${dateValue}" \
  https://${bucket}.s3.amazonaws.com/${key}/${fileName}
